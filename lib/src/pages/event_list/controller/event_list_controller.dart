...
import '../repository/event_list_repository.dart';
import 'dart:math';
class EventListController extends GetxController {
  final RxList<EventViewModel> events = RxList();
  RxBool isLoading = true.obs;
  RxBool isRetry = false.obs;
  final EventListRepository repository = EventListRepository();
  ...
  void goToDetails(int id, bool isNotExpired) async{
    if (isNotExpired) {
  ...
  void bookmarkEvent(EventViewModel model) async {
    if (model.isBookmark) {
      unBookmark(model);
    } else {
      bookmark(model);
  ...
  void bookmark(EventViewModel model) async {
    bookmarkLoading[model.id.toString()] = true;
    final pref = await SharedPreferences.getInstance();
    final responseOrException = await repository.bookmarkEvent(BookmarkEventDto(
      image: model.image,
      title: model.title,
  ...
  void unBookmark(EventViewModel model) async {
    bookmarkLoading[model.id.toString()] = true;
    ...
    responseOrException.fold((left) {
      ...
      if (index != -1) {
        events[index] =
            events[index].copyWith(isBookmark: !events[index].isBookmark);
      }
      showToast(LocaleKeys.shared_text_success.tr, false);
    });
  }
